...
namespace Fogo.Api.Controllers {
    ...
    public class ReadOnlyController<TService, TModel, TViewModel> :
        ControllerBase
        where TService : IReadOnlyService<TModel, TViewModel>
        ...
        [HttpGet("{id}")]
        public virtual async Task<IActionResult> Get([FromRoute, Bind("Id")] TViewModel viewModel, CancellationToken cancellationToken = default) {
            try {
                return Ok(await _service.ReadAsync(viewModel, cancellationToken));
            } catch (Exception exception) {
                _logger.LogError(exception, exception.Message);
